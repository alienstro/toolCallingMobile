...
class MemoryRepositoryTest {
    ...
    fun upsertReplacesExistingKeyAndMovesItToMostRecent() {
        ...
        repository.upsertMemory("project.current", "LlamaCpp Android app", now = 3000L)

        assertEquals(
            listOf(
                MemoryItem("project.current", "LlamaCpp Android app", 3000L),
    ...
    fun selectForPromptIncludesPinnedRelevantThenRecentMemories() {
        val repository = MemoryRepository(temporaryFolder.root)
        repository.upsertMemory("project.current", "LlamaCpp Android app", now = 1000L)
        repository.upsertMemory("user.name", "Lance", now = 2000L)
        ...
        assertEquals(
            listOf(
                ...
                MemoryItem("project.current", "LlamaCpp Android app", 1000L),
